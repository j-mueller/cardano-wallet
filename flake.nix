      ...
      (system:
        let
          ...
          packages = project:
            ...
            // (prefixNamesWith "checks/" (project.flake flakeArgs).checks)
          ...
          hydraJobs = (packages hydraProject)
          // (flattenTree {
            inherit (flake) devShell;
            devShells = pkgs.recurseIntoAttrs devShells;
            ...
            roots = pkgs.recurseIntoAttrs
              ...
              windowsProject = hydraProject.projectCross.mingwW64.roots;
            });
          })
          // (lib.optionalAttrs buildPlatform.isLinux (
            let
            ...
            in
            {
              ...
              cardano-wallet-tests-win64 = import ./nix/windows-testing-bundle.nix {
                ...
                tests = lib.attrValues (lib.filterAttrs (n: _: lib.hasInfix ":test:" n) windowsPackages);
                benchmarks = lib.attrValues (lib.filterAttrs (n: _: lib.hasInfix ":benchmarks:" n) windowsPackages);
              };
                ...
                constituents = let hj = self.hydraJobs; in
                  lib.concatMap lib.attrValues ([
                    ...
                    (n: _:
                      ...
                      || (lib.hasInfix ":benchmarks:" n && !(lib.hasPrefix "x86_64-unknown-linux-musl:" n))
                    )
                    hj));
              };
            }
            ...
            cardano-wallet-macos64 = import ./nix/release-package.nix {
              inherit pkgs;
              exes = releaseContents (packages hydraProject);
              platform = "macos64";
              format = "tar.gz";
            };
          });
