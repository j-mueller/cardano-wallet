  ...
  ({ pkgs
   ...
   , ...
    ...
    let
      ...
      setGitRevPostInstall = ''${pkgs.buildPackages.iohk-nix-utils}/bin/set-git-rev "${config.gitrev}" $out/bin/*'';
    ...
    in
    {
      ...
      modules =
        let inherit (config) src coverage profiling doCheck doIntegrationCheck;
          projectPackages = lib.attrNames (lib.filterAttrs
            (_: p: p != null
